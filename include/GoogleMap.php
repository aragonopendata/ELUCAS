	...
	*/
    ...
    function getMapJS() {
	   ...
	   function getMapJS2() {
        ...
        // create global info window ( so we can auto close it )
        $_script .= " infowindow = new google.maps.InfoWindow();";
			        
        
		if($this->street_view_dom_id!=""){
			$_script .= "
				 panorama".$this->street_view_dom_id."$_key = '';
			";
			if(!empty($this->_markers)){
				$_script .= "
					 panorama".$this->street_view_dom_id."markers$_key = [];
				";
			}
		}
        ...
        $_script .= " map$_key = null;\n";
		
		//start setting script var
        if($this->onload) {
           $_script .= 'function load'.$this->map_id.'() {' . "\n";   
        } 
		...
		if($this->display_map){
			$_script .= sprintf(' mapObj%s = document.getElementById("%s");', $_key, $this->map_id) . "\n";
			$_script .= "if (mapObj$_key != 'undefined' && mapObj$_key != null) {\n";
			...
			if($this->zoom_encompass && (count($this->_markers) > 1 || count($this->_polylines) >= 1 || count($this->_overlays) >= 1)) {
				...
				$_script .= " bds$_key = new google.maps.LatLngBounds(new google.maps.LatLng($this->_min_lat, $this->_min_lon), new google.maps.LatLng($this->_max_lat, $this->_max_lon));\n";
				$_script .= 'map'.$_key.'.fitBounds(bds'.$_key.');' . "\n";
			}
